...
class TestSupervisedLearning(SimpleTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.slt, _ = SupervisedLearningTechnique.objects.get_or_create(
    ...
    def test_get_targets(self):
        ...
        expected_output = [
            label
            for label in sl_test_models.UserInfo.objects.values_list(
                "sex", flat=True
            )
        ]
    ...
    def test_predict(self):
        ...
        with mock.patch.object(
            slt, "engine_object_predict", return_value=[[0]]
        ):
    ...
    def test_through_classifiers(self):
        ...
        for classifier in classifiers:
            ...
            p1, s1 = classifier.predict(
                [{"avg1": 1.2, "cluster_1": None}], include_scores=True
            )
    ...
    def test_svm_imputer(self):
        ...
        self.assertEqual(output, expected_output)
        expected_output = {"bool_field": False, "cluster_1": 0}
        output = slt.impute(
            sl_test_models.UserInfo(avg1=14.224856508534689, cluster_2=1)
        )
        self.assertEqual(output, expected_output)
        ...
        output = slt.impute(sl_test_models.UserInfo(bool_field=False))
        self.assertEqual(output, expected_output)
        with self.assertRaises(ValueError):
    ...
    def test_sl_validation(self):
        ...
        with self.assertRaises(ValidationError):
            slt.full_clean()
        with mock.patch.object(
            sl_test_models.UserInfo, "LEARNING_TARGET", "avgX"
        ):
            slt.learning_target = None
            with self.assertRaises(ValidationError):
                slt.full_clean()
        with mock.patch.object(
            sl_test_models.UserInfo, "LEARNING_TARGET", None
        ):
            slt.learning_target = None
            with self.assertRaises(ValidationError):
                slt.full_clean()

...
class HGBTreeRegressor(SupervisedLearningTechnique):
    """
    Histogram-based Gradient Boosting Tree - Regression
    ...
    loss = models.CharField(
        ...
        help_text=(
            _(
                ...
                "If none is given, least_squares will be used."
            )
        ),
    )
    ...
    validation_fraction = models.FloatField(
        # In classificaiton, not in regression
    ...
    class Meta:
        verbose_name = _("HGB Tree for Regression")
        verbose_name_plural = _("HGB Trees for Regression")
    ...
    def __str__(self):
        return "[HGBTree|R] {0}".format(self.name)
    ...
    def get_inference_scores(self):
        ...
        labels = self.get_targets()
        eo = self.get_engine_object()
        scores["r2"] = float(eo.score(data, labels))
        return scores
    ...
    def _get_metadata_descriptions(self):
        ...
        descriptions["r2"] = "Coefficient of Determination R^2"
        return descriptions
